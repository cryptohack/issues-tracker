        ...
        while True:
            ...
            if hasattr(c, 'no_prompt'):
                ...
                if hasattr(c, 'max_payload_size'):
                    try:
                        self.datas = b""
                        while len(self.datas) < max_recv_size:
                            chunk = self.request.recv(1024)
                            if not chunk:
                                break
                            self.datas += chunk.strip()
                            if b"\n" in chunk:
                                break
                    ...
                    try:
                        self.datas = self.request.recv(max_recv_size).strip()
                    except ConnectionResetError:
                        break
                if len(self.datas) >= max_recv_size:
                ...
                if self.datas:
                    self.log(self.datas)
                try:
                    datas = map(json.loads, self.datas.split(b"\n"))
